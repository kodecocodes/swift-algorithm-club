...
唯一能够保证的是基准左边的元素都比基准小，右边的都比基准大。由于划分会改变相同元素的原始顺序，所以快速排序不是一个“稳定”的排序（例如：[归并排序](../Merge Sort/README-CN.markdown)）。大部分时候这都不是一个大问题。
...
如果数组中包含很多的重复元素的话，用荷兰国旗分块可以让快速排序更高效（这不是以为我是荷兰人才这样说）。
